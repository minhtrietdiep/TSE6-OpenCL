...
void loadppm_toU8RGB(const char *fname, unsigned int* pW, unsigned int* pH, U8 **img, unsigned int *upImgSize)
{
	...
	*img = 0;
	...
	if (pImg == NULL)
	{
		printf("loadppm_toU8RGB: Couldn't allocate memory of size: %d in file %s: %s\n", uImgSize, fname);
		exit(-1);
	}
